...
const collectionId = "pbcnilaitahfizh1";
const collectionName = "nilai_tahfizh";
...
const fields = [
  ...
  },
  {
    ...
    system: false,
    type: "number",
    max: 1000, // Just a reasonable upper limit for ayat
    min: 0,
    noDecimal: false
  },
  {
    hidden: false,
    id: "autodate_nilaitahf_created",
    name: "created",
    onCreate: true,
    onUpdate: false,
    presentable: false,
...
runSql(`create unique index if not exists idx_nilaitahfizh_siswa_materi on ${collectionName}(siswa, materi);`);
...
const values = [
  ...
  sqlString(JSON.stringify(["create unique index idx_nilaitahfizh_siswa_materi on nilai_tahfizh (siswa, materi)"])),
...
console.log("Collection nilai_tahfizh siap digunakan.");
